...
def generate_launch_description():
    ...
    def setRobotId(context):
        ...
        robot_config = os.path.join(get_package_share_directory('multibot_arch'), 'config', 'nav2', f'nav2_localization_kobuki_{id}.yaml')
    ...
    def local_config_path(context):
        ...
        config_file_path = os.path.join(get_package_share_directory('multibot_arch'), 'config', 'dds_partitions','robot_local_debug_config.xml')
    ...
    def shared_config_path(context):
        ...
        # Get configuration file
        config_file_path = os.path.join(get_package_share_directory('multibot_arch'), 'config', 'dds_partitions','robot_shared_config.xml')
        target_path = createConfigFile(config_file_path, robot_id)
    ...
    localization = GroupAction([
        ...
        IncludeLaunchDescription(
            ...
            launch_arguments={
                ...
                'map': os.path.join(get_package_share_directory('multibot_arch'), 'map', 'map.yaml'),
    ...
    nav2 = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory('multibot_arch'),
                'launch/nav/navigation_launch.py')),
        launch_arguments={
            "namespace": LaunchConfiguration('namespace'),
            "params_file": os.path.join(get_package_share_directory('multibot_arch'), 'config', 'nav2', 'nav2_params_kobuki.yaml'),
            'log_level': LaunchConfiguration('nav_log_level')
        }.items()
    )
