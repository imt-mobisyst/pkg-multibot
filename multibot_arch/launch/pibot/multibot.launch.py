...
def tbot(context):
    ...
    match(type):
        ...
        case "partitions":
            ...
            config_file_path = os.path.join(get_package_share_directory('multibot_arch'), 'config', 'dds_partitions','robot_local_config.xml')
            ...
            return [GroupAction([
                ...
                ExecuteProcess(
                    cmd=[[
                        ...
                        ' -c ',
                        os.path.join(get_package_share_directory('multibot_arch'), 'config', 'zenoh', 'kobuki', 'bridge_config_pibot.json5')
                    ]],
                    shell=True
                ),
    ...
    print("Wrong multibot_arch type, must be one of the following : 'namespace', 'domain_id', 'discovery', 'partitions', 'zenoh'")
    return []



def generate_launch_description():
    multibot_arch_type_launch_arg = DeclareLaunchArgument(
        'type', description="Must be one of the following : 'namespace', 'domain_id', 'discovery' 'partitions', 'zenoh'", default_value=''
    )

    return LaunchDescription([
        multibot_arch_type_launch_arg,
