    ...
    simulator = GroupAction([
        ...
        ExecuteProcess(
            cmd=[[
                ...
                ' -c ',
                os.path.join(get_package_share_directory('multibot'), 'config', 'zenoh', 'stage', 'bridge_config_sim.json5')
            ]],
            shell=True
        )
    ])
        ...
        GroupAction([
            ...
            ExecuteProcess(
                cmd=[[
                    FindExecutable(name='zenoh-bridge-ros2dds'),
                    ' -c ',
                    os.path.join(get_package_share_directory('multibot'), 'config', 'zenoh', 'stage', 'bridge_config_operator.json5')
                ]],
                shell=True
            )
        ])
