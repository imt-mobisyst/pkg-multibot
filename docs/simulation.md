...
```bash
ros2 launch multibot_arch turtlesim_namespace_launch.py nb_robots:="3"
```
...
```bash
ros2 launch multibot_arch rviz_launch.py config:=config/stage.rviz
ros2 launch multibot_arch stage_namespace_launch.py
```
...
```bash
ros2 launch multibot_arch turtlesim_bridge_launch.py nb_robots:="3"
```
---
...
```bash
ros2 launch multibot_arch turtlesim_bridge_robot_launch.py bot_domain_id:="10" operator_domain_id:="1"
ros2 launch multibot_arch turtlesim_bridge_robot_launch.py bot_domain_id:="11" operator_domain_id:="1"
```
...
```bash
ROS_DOMAIN_ID=1 ros2 run multibot_arch static_operator.py --ros-args -p nb_robots:=2
```

Running `rviz` in the operator's `ROS_DOMAIN_ID` :
```bash
ROS_DOMAIN_ID=1 ros2 launch multmultibot_archibot rviz_turtlesim_launch.py
```
...
```bash
export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp
ros2 launch multibot_arch stage_bridge_launch.py
```
...
To start the demo, you can use the following command, that will take care of starting everything (`turtle`, `operator` et `rviz`) :
```bash
ros2 launch multibot_arch turtlesim_dds_launch.py nb_robots:="3"
```
...
```bash
ros2 launch multibot_arch turtlesim_dds_robot_launch.py local_dds_server:="127.0.0.1:11811" subnet_dds_server:="127.0.0.1:11814" nb_robots:="3" robot_id:="1"
ros2 launch multibot_arch turtlesim_dds_robot_launch.py local_dds_server:="127.0.0.1:11812" subnet_dds_server:="127.0.0.1:11814" nb_robots:="3" robot_id:="2"
ros2 launch multibot_arch turtlesim_dds_robot_launch.py local_dds_server:="127.0.0.1:11813" subnet_dds_server:="127.0.0.1:11814" nb_robots:="3" robot_id:="3"
```

Starting the operator node :
```bash
export ROS_DISCOVERY_SERVER=";;;127.0.0.1:11814"
ros2 run multibot_arch static_operator.py --ros-args -p nb_robots:=3
```

Starting rviz :
```bash
export ROS_DISCOVERY_SERVER=";;;127.0.0.1:11814"
ros2 launch multibot_arch rviz_turtlesim_launch.py
```
...
On the operator PC :
```bash
ros2 launch multibot_arch pibot_dds_operator.py nb_robots:="2" common_dds_ip:="10.89.5.90" common_dds_port:="11811"
```

On each *pibot* :
```bash
ros2 launch multibot_arch pibot_turtlesim_dds_launch.py nb_robots:=2 operator_server:="10.89.5.90:11811"
```
...
```bash
ros2 launch multibot_arch stage_dds_launch.py
```
...
To start the demo, you can use the following command, that will take care of starting everything (`turtle`, `operator` et `rviz`) :
```bash
ros2 launch multibot_arch turtlesim_partition_launch.py nb_robots:="3"
```
...
```bash
ros2 launch multibot_arch turtlesim_partition_robot_launch.py nb_robots:="3" robot_id:="1"
ros2 launch multibot_arch turtlesim_partition_robot_launch.py nb_robots:="3" robot_id:="2"
ros2 launch multibot_arch turtlesim_partition_robot_launch.py nb_robots:="3" robot_id:="3"
```
...
In another terminal, launch the demo (with the simulator, the controllers, the nav2 stacks...):
```bash
ros2 launch multibot_arch stage_partition_launch.py
```
...
To launch rviz, run the following commands :
```bash
ROS_DOMAIN_ID=99 ros2 launch multibot_arch rviz_launch.py config:=config/stage.rviz
```

In another terminal, launch the demo (with the simulator, the controllers, the nav2 stacks...):
```bash
ros2 launch multibot_arch stage_zenoh_launch.py
```
